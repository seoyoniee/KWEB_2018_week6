...
exports.rentBooks = async (req,res) => {
    try {
...
exports.updateBook = async (req, res) => {
    try {
        ...
        if(isRent != NULL) {
            await processQuery('UPDATE `books` SET title = ?, author = ?, publish = ?, isRent = ?, comment = ? WHERE id = ?', [title, author, publish, isRent, comment, id]);
        }
        else
        {
